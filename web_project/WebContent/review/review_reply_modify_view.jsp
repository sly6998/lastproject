...
<%
  ...
  ReviewBean review = (ReviewBean) request.getAttribute("ssibal");
  ReviewBean modify_reply = (ReviewBean) request.getAttribute("modify_reply");
%>
...
<body>
    ...
    <section id="content">
      <div class="container">
        <div class="row">
          <div class="col-lg-12">
            <!-- 본문 시작 -->
            <h3 class="heading">REVIEW게시판</h3>
            <br> <br>
            <form method="post" action="./review_modify_view.html">
              <input type="hidden" name="REVIEW_NUM" value="<%=review.getREVIEW_NUM()%>">
              <table class="table">
                <tr>
                  <td width="15%" bgcolor="eeeeee">제목</td>
                  <td width="85%"><%=review.getREVIEW_SUBJECT()%></td>
                </tr>
                <tr>
                  <td width="15%" bgcolor="eeeeee">작성자</td>
                  <td width="85%"><%=review.getREVIEW_MEMBER_NAME()%></td>
                </tr>
                <tr>
                  <td width="15%" bgcolor="eeeeee">작성일</td>
                  <td width="85%"><%=review.getREVIEW_DATE()%></td>
                </tr>
                <tr>
              ...
              <div align="center">
                <%
                  if (id.equals(review.getREVIEW_REPLY_MEMBER_ID()) || id.equals("admin")) {
                %>
                <input class="btn" type="submit" value="수정">&nbsp;&nbsp; <input class="btn" type="button" value="삭제"
                  onclick="location.href='./ReviewDeleteAction.html?REVIEW_NUM=<%=review.getREVIEW_NUM()%>'">&nbsp;&nbsp;
                <%
                  }
                %>
                <input class="btn" type="button" value="목록" onclick="location.href='./review_list.html'">
              </div>
            </form>
            ...
            <form>
              <%
                for (int i = 0; i < replylist.size(); i++) {
                  ReviewBean rl = (ReviewBean) replylist.get(i);
              %>
              <table width="100%">
                <tr style="border-bottom: 1px;">
                  <td width="10%"><b><%=rl.getREVIEW_REPLY_MEMBER_ID()%></b></td>
                  <td><%=rl.getREVIEW_REPLY_DATE()%></td>
                  <td align="right">
                    <%
                      if (id.equals(rl.getREVIEW_REPLY_MEMBER_ID()) || id.equals("admin")) {
                    %> <a style="text-decoration: none;" href='./ReviewReplyModifyViewAction.html?REVIEW_NUM=<%=review.getREVIEW_NUM()%>&REVIEW_REPLY_SEQ=<%=rl.getREVIEW_REPLY_SEQ()%>'>수정&nbsp;&nbsp;&nbsp;</a> <a
                    style="text-decoration: none;" href='./ReviewReplyDeleteAction.html?REVIEW_NUM=<%=review.getREVIEW_NUM()%>&REVIEW_REPLY_SEQ=<%=rl.getREVIEW_REPLY_SEQ()%>'>삭제&nbsp;&nbsp;&nbsp;</a> <%
   }
 ...
 %> <a style="text-decoration: none;" href="#">답글&nbsp;&nbsp;&nbsp;</a>
                ...
                <tr>
                  <td colspan="3"><br><%=rl.getREVIEW_REPLY_CONTENT()%></td>
                </tr>
              </table>
            ...
            <table width="100%">
              ...
              %>
              <form name="review_reply_modify_form" method="post" action="./ReviewReplyModify.html">
                <tr>
                  <td><br> <br></td>
                </tr>
                <tr>
                  <td width="10%"><input type="hidden" name="REVIEW_REPLY_MEMBER_ID" value="<%=id%>" /> 
                  <input type="hidden" name="num" value="<%=review.getREVIEW_NUM()%>" /> 
                  <b><%=id%></b></td>
                  <td width="80%"><textarea name="REVIEW_REPLY_CONTENT" id="REVIEW_REPLY_CONTENT" type="text" style="width: 100%; height: 150px; resize: none;"><%=modify_reply.getREVIEW_REPLY_CONTENT() %></textarea></td>
                  <input type="hidden" name="REVIEW_REPLY_SEQ" value="<%=modify_reply.getREVIEW_REPLY_SEQ()%>">
