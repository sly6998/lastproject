...
function infocus(element){
 ...
 function sub(){
	 ...
	 if(document.getElementById('MEMBER_ID').value=='' || !id_re.test(document.getElementById('MEMBER_ID').value)){
		...
		window.open("./MemberFindPWDAction.html?MEMBER_ID="+id+"&MEMBER_NAME="+name+"&MEMBER_TEL="+tel+"&MEMBER_EMAIL="+email+"&MEMBER_YEAR="+year+"&MEMBER_MONTH="+month+"&MEMBER_DAY="+day,"",'left='+(screen.availWidth-300)/2+',top='+(screen.availHeight-120)/4+', width=300,height=120px');
